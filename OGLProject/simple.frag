...
void main()
{
	...
	//Attenuation
	float dis = length(lightPos0 - vs_position);
	float attenuation = 1.0f / (1.0f + 0.09f * dis + 0.032f * (dis * dis));

	//Final light
	fs_color = 
	texture(material.diffuseTex, vs_texcoord)
	* (attenuation * (vec4(ambientFinal, 1.f) + vec4(diffuseFinal, 1.f) + vec4(specularFinal, 1.f)));
}
